...
def hermes_task_context(config: RelayConfig, run_dir: Path, *, write: bool = False) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "editable_files": ["scripts/final.py", "scripts/browser-use.py"],
        ...
        "contract": {
            ...
            "browser_use": "Optional bounded Browser Use CLI bridge accepts only the cooperative read-only AST subset; it is not a sandbox or redirect firewall.",
        ...
        "commands": {
            ...
            "captcha_inspect": f"scripts/chip-relay task captcha {run_id} inspect",
            "captcha_wait": f"scripts/chip-relay task captcha {run_id} wait --timeout 120",
            "captcha_resume": f"scripts/chip-relay task captcha {run_id} resume",
        ...
        "browser_use": evidence["browser_use"],
        "evidence": evidence,
        "artifacts": {
