...
Each default `browser-use` execution gets a fresh owner-only `BH_RUNTIME_DIR` and `BH_TMP_DIR`, a unique `BU_NAME`, and the configured Relay `BU_CDP_URL`; it cannot reuse a pre-existing global Browser Harness daemon. Before success, Relay pings that isolated daemon, requires `browser_kind=cdp`, probes `Browser.getVersion`, then shuts the daemon down and verifies that both process and endpoint are gone. Same-process-group descendants are killed even when the direct CLI parent exits cleanly. A configured custom command remains explicitly labeled `configured-command-unattested`; only the installed `browser-use` runner can produce `isolated-daemon-cdp` / `attested-and-closed`.

Screenshot import is additionally bound to the validated AST: without an explicit `capture_screenshot()` call, printed image paths are ignored. With that helper present, Relay accepts only a structurally valid PNG created inside that execution's fresh `BH_TMP_DIR`, copies it to run-private `screenshots/`, and removes the temporary execution directory after daemon closure.
